...
`tensor.pack` 是 MLIR Linalg dialect 中用于**数据布局重排**（Data Layout Relayout）的核心操作。它通过将张量的某些维度分块（tiling）并重新排列，实现三大性能优化目标（**不仅仅是这3个**）：
...
通过 `tensor.pack`，MLIR 提供了一种优雅的方式，在**不改变算子语义**的前提下，**显式地控制数据的物理布局**，从而精确的解决现在处理器的性能瓶颈问题。
...
`tensor.pack` 通过**数据布局重排**实现三大性能优化：
