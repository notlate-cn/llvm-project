...
## 0. 关于融合（Fusion）的定义（Fusion Primer）

### Fusion的本质

在保持语义等价的前提下，

- 改变 **计算边界（Compute Boundary）**
- 重组 **执行时序（Execution Order）**
- 重映射 **数据生存期（Data Lifetime）**

**以最小化 *Data Movement* 并最大化 *Hardware Utilization***

### Fusion ≠ 合并算子

### Fusion = **边界消失 + 生命周期延长 + 调度重排**
...
在 GPU 与 Ascend NPU 这类现代 AI 加速器上，性能并非只由“算了多少 FLOPs”决定，而是由**寄存器、片上缓冲区、并行度与专用计算单元**之间的微妙平衡所主导。
...
#### 7.1.2 Implicit Mask Fusion (On-the-fly Mask Generation)
