# MLIR Linalg ElementwiseOpFusion 深度解析
...
graph TD
    ...
    A --> C[构建fusedInputOperands]
    C --> C1[consumer inputs before fusedOperand]
    C --> C2[producer inputs + 转换IndexingMap]
    C --> C3[consumer inputs after fusedOperand]

    A --> D[构建fusedOutputOperands]
    D --> D1[preserved producer outputs]
    D --> D2[consumer outputs]

    A --> E[create GenericOp]
...
| **Producer语义** | 必须是纯tensor语义 | `ElementwiseOpFusion.cpp:153` | 避免memref别名问题 |
...
| **可逆性** | Producer result map必须是permutation | `ElementwiseOpFusion.cpp:177` | 需要计算inverse map |
...
| **外部使用** | 除consumer外还有其他用户 | `ElementwiseOpFusion.cpp:126-128` | 结果被其他op使用 |
