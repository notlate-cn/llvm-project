...
### 机制1：Transform Dialect（变换方言）- 让专家"指导"编译器
...
### 机制2：高阶特定算子（Named Op）与分解（Decomposition）
...
### 机制3：微内核架构 (Micro-kernels / UKernels)
...
> **注意**：上述代码片段均为**概念性展示**，用于说明每种机制的工作原理。完整的实现示例请参见后续的"完整示例"章节。
...
func.func @tiled_attention(
    %Q: tensor<1024x1024xf16>,
    %K: tensor<1024x1024xf16>,
    %V: tensor<1024x1024xf16>
) -> tensor<1024x1024xf32> {

    // 外层循环: 分块处理
    %O_final = scf.for %block_i = %c0 to %num_blocks step %c1
        iter_args(%O_acc) = (%O_init) {

        // 提取当前块
        %Q_block = tensor.extract_slice %Q[%offset, 0] [64, 1024] [1, 1]
        %K_block = tensor.extract_slice %K[0, %offset] [1024, 64] [1, 1]

        // 块内计算 (可以放入共享内存)
        %S_block = linalg.matmul
            ins(%Q_block, %K_block)
            outs(%S_init)
...
// 初始化统计量
%m = linalg.generic { index = [0] }
    ins(%input : tensor<64x64xf32>)
    outs(%m_init : tensor<64xf32>) {
    ^bb0(%x: f32, %acc: f32):
        %max = arith.maxf %x, %acc : f32
        linalg.yield %max : f32
} -> tensor<64xf32>

// 循环更新
%m_new, %l_new, %O_new = scf.for %block = %c0 to %num_blocks step %c1
    iter_args(%m_acc, %l_acc, %O_acc) = (%m, %l, %O) {

    // 1. 计算当前块的softmax
    %S_shifted = arith.subf %S_block, %m_acc_broadcast
    %P = math.exp %S_shifted

    // 2. 更新统计量
    %m_new = vector.reduce <max>, %m_acc, %S_block
    %P_sum = vector.reduce <add>, %P
    %l_new = arith.addf %l_acc, %P_sum

    // 3. 更新输出
    %l_ratio = arith.divf %l_acc, %l_new
    %O_scaled = arith.mulf %O_acc, %l_ratio
    %O_new = arith.addf %O_scaled, %P

    scf.yield %m_new, %l_new, %O_new
}
```
...
时间线:
─────────────────────────────────────────>
Stage 0: | Load K0/V0 | Compute QK0^T | Store O0 |
Stage 1:              | Load K1/V1    | Compute QK1^T |
         ↑            ↑               ↑
      Memory        Overlap         Compute
```
...
// 流水线实现 (概念)
// ============================================
func.func @pipelined_attention(...) {
    // 双缓冲: stage 0和stage 1
    %K_shared = memref.alloc() : memref<2x64x64xf16, 3>
    %V_shared = memref.alloc() : memref<2x64x64xf16, 3>

    %O_final = scf.for %block = %c0 to %num_blocks step %c1
        iter_args(%O_acc) = (%O_init) {

        %stage = arith.remui %block, %c2 : index

        // 异步加载下一块 (非阻塞)
        %next_offset = arith.muli %block, %c64
        gpu.async_copy %K_global[%next_offset], %K_shared[%stage, ...]
        gpu.async_copy %V_global[%next_offset], %V_shared[%stage, ...]

        // 等待当前块数据就绪
        gpu.barrier

        // 使用上一stage的数据计算
        %compute_stage = arith.subi %stage, %c1
        %S = linalg.matmul
            ins(%Q, %K_shared[%compute_stage, ...])
...
// 高层: 矩阵乘
%C = linalg.matmul
    ins(%A: tensor<64x64xf16>, %B: tensor<64x64xf16>)
    outs(%C_init: tensor<64x64xf32>)

// ↓ 编译器自动转换

// 中层: 向量化操作
%C_vec = vector.contract %A_vec, %B_vec, %C_vec
    : vector<16x16xf16>, vector<16x16xf16> -> vector<16x16xf32>

// ↓ 后端自动转换

// 低层: 硬件指令 (如Tensor Core)
nvgpu.mma.sync %C, %A, %B, %C
    : vector<16x16xf16>, vector<16x16xf16> to vector<16x16xf32>
```
...
| 位置               | 代码类型 | 说明                                       |
| ------------------ | -------- | ------------------------------------------ |
| **"核心理念"章节** | 片段代码 | 展示每种机制的**独立用法**，用于理解概念   |
| **"完整示例"章节** | 完整流程 | 展示四种机制如何**组合使用**，从高层到底层 |

**对应关系**：

- "核心理念"的机制2（分解） → "完整示例"的Step 2（在线算法展开）
- "核心理念"的机制1（Transform） → "完整示例"的Transform脚本
- "核心理念"的机制4（硬件方言） → "完整示例"的gpu/nvgpu操作
